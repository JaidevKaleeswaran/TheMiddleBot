...
const BidsList = () => {
    ...
    const [filterHighest, setFilterHighest] = useState(false);

    const filteredBids = mockBids.filter(bid => {
        const q = searchQuery.toLowerCase();
        const matchesSearch = bid.bidderName.toLowerCase().includes(q) || bid.property.toLowerCase().includes(q);
        const matchesFilter = !filterHighest || bid.isHighest;
            ...
            <div className="bg-surface-card rounded-2xl border border-slate-700/50 overflow-hidden">
                <div className="p-4 bg-slate-800/20 border-b border-slate-800/80 flex flex-col md:flex-row gap-4 items-center">
                    ...
                    <div className="flex gap-2 w-full md:w-auto">
                        <button
                            onClick={() => setFilterHighest(!filterHighest)}
                            className={`flex-1 md:flex-none px-4 py-2 border rounded-xl text-sm font-bold flex items-center justify-center gap-2 transition-all ${filterHighest
                                    ? 'bg-brand-500/20 text-brand-300 border-brand-500/30 shadow-lg shadow-brand-500/10'
                                    : 'bg-brand-500/10 text-brand-400 border-brand-500/20'
                                }`}
                        >
                            <TrendingUp size={16} /> {filterHighest ? 'Showing Highest Only' : 'Filter High Bids'}
                        </button>
                    </div>
                </div>
                                    ...
                                    <td className="px-6 py-4 text-right">
                                        <div className="flex items-center justify-end gap-2">
                                            ...
                                            <Link
                                                ...
                                                className="px-4 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-lg text-xs font-bold border border-slate-700 transition-colors"
                                            >
                                                Details
                                            </Link>
                                        </div>
